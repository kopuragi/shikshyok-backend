...
const storage = multer.diskStorage({
  destination: path.join(__dirname, '../src/assets'), // src/assets/uploads로 설정
...
// 사용자 프로필 수정 핸들러
(exports.updateUserProfile = upload.single('profileImage')),
  async (req, res) => {
    const {
      nickname,
      email,
    ...
    try {
      ...
      return res.status(200).json({
        ...
        user: {
          ...
          membershipType: user.membershipType,
          profileImage: user.profileImage, // 프로필 이미지 경로 추가
        },
      });
    } catch (error) {
